...
df <- df |> mutate(precio = str_replace(precio, "\\.", ""))
...
df_filtered$marca <- str_replace_all(df_filtered$marca, "\\s+", "")
...
df_filtered <- df_filtered |> 
  ...
  mutate(marca = str_replace_all(marca, "AppleSamsungHuawei", ""))
  

df_filtered <- df_filtered |>
  mutate(marca = str_replace_all(marca, "^$","Otros"))
